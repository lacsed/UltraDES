...
void ITL(out List<DeterministicFiniteAutomaton> plants, out List<DeterministicFiniteAutomaton> specs)
{
    var s =
...
// The automata are saved in 'plants' and 'specs'
void ClusterTool(int clusters, out List<DeterministicFiniteAutomaton> plants, out List<DeterministicFiniteAutomaton> specs)
{
    ...
    for (var i = 1; i < clusters; i++)
    {
        var e61 = new Event($"{i}|6", Controllability.Uncontrollable);
        var e31 = new Event($"{i}|3", Controllability.Controllable);
        var e12 = new Event($"{i + 1}|1", Controllability.Controllable);
        var e42 = new Event($"{i + 1}|4", Controllability.Uncontrollable);
...
// The automata are saved in 'plants' and 'specs'
void FMS(out List<DeterministicFiniteAutomaton> plants, out List<DeterministicFiniteAutomaton> specs, bool conflicting = false)
{
    var s =
        Enumerable.Range(0, 6)
            .Select(i =>
...
void TestComputingMonolithicSupervisor()
{
    // Choose one option below
    ClusterTool(5, out var plants, out var specs);
...
void TestComputingModularSupervisor()
{
    // Choose one option below
...
void TestComputingModularReducedSupervisor()
{
    // Choose one option below
    FMS(out var plants, out var specs);
